...
export default function HomeInteriors() {
    ...
    const fetchTopDesigners = useCallback(async (city) => {
        try {
            const res = await fetch(`${API}/api/interior-designers/top?city=${encodeURIComponent(city || DEFAULT_CITY)}&limit=${TOP_LIMIT}`);
            const data = await res.json();
    ...
    useEffect(() => { fetchTopDesigners(selectedCity); }, [selectedCity, fetchTopDesigners]);
                    ...
                    <div className="hi-modal-overlay" onClick={() => setSelectedArticle(null)}>
                        <div className="hi-modal-content" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="hi-modal-img">
                                <img src={resolveImageUrl(selectedArticle.image)} alt={selectedArticle.title} />
