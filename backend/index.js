...
// Chat endpoint: persists messages and returns assistant reply as a small stream
app.post('/api/chat', async (req, res) => {
  try {
    // Optional auth: if Authorization header present, authenticate and attach user
    const authHeader = req.headers['authorization'];
    let authUser = null;
    if (authHeader) {
      ...
      if (token) {
        try {
          ...
          // ignore; treat as anonymous if token invalid
        }
      }
    }
    ...
    const Conversation = ChatConversation || sequelize.models.ChatConversation;
    const MessageModel = ChatMessage || sequelize.models.ChatMessage;
    ...
    if (!conversation) {
      conversation = await Conversation.create({ title: 'Assistant Chat', userId: authUser ? authUser.id : null });
    }

    // Save user message
    await MessageModel.create({ conversationId: conversation.id, userId: authUser ? authUser.id : null, sender: 'user', message });

    // Determine welcome message (customizable via Settings)
    let settings = await Settings.findOne();
    let welcome = 'Hello! How can I help you with interiors, calculators, or our services today?';
    if (settings && settings.chatWelcomeMessage) welcome = settings.chatWelcomeMessage;

    // If this conversation already has assistant messages, reply with a short canned response
    const existingAssistant = await MessageModel.findOne({ where: { conversationId: conversation.id, sender: 'assistant' } });
    let assistantText = existingAssistant ? `Thanks — we received your message. Our team will respond shortly.` : welcome;

    // Save assistant message
    await MessageModel.create({ conversationId: conversation.id, sender: 'assistant', message: assistantText });

    // Stream a single JSON chunk and end. ChatBox adapter expects a ReadableStream; sending chunked JSON works.
    res.setHeader('Content-Type', 'application/octet-stream');
    const payload = JSON.stringify({ type: 'message', conversationId: conversation.id, text: assistantText });
    res.write(payload);
    res.end();
  } catch (error) {
